class EloService
  ...
  def self.elo_score_generator(winner, loser, sport)
    ...
    winner.sport_elo_ratings.sport(sport.sport).elo_rating = winner_rating + ELO_PARAM * (1.00 - winner_expected)
    loser.sport_elo_ratings.sport(sport.sport).elo_rating = loser_rating + ELO_PARAM * (0.00 - loser_expected)
  end
end